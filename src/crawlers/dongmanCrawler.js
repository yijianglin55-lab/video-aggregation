...
class DongmanCrawler {
   ...
   */
  static async getDongmanList(params = {}) {
    try {
  ...
  static parseList($) {
    ...
    $('.myui-vodbox-content').each((i, el) => {
      ...
      if (!urlPath.startsWith('/dongman/')) return;
      ...
      const id = urlPath.replace('/dongman/', '').replace(/\/$/, '');
